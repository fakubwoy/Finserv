...
def process_data(data):
    ...
    for item in data:
        if item.isdigit() or (item.startswith('-') and item[1:].isdigit()):
            ...
            for char in item:
                result["all_alphabets"].append(char.lower())
        else:
...
def create_concat_string(all_alphabets):
    ...
    for i, char in enumerate(reversed_chars):
        concat_result += char.upper() if i % 2 == 0 else char.lower()
...
def process_bfhl():
    try:
        ...
        if not request_data or 'data' not in request_data:
            return jsonify({"is_success": False, "error": "Invalid input. 'data' field is required."}), 400
        ...
        response = {
            ...
            "sum": str(processed["sum"]),
            "concat_string": concat_string
        }
        return jsonify(response), 200
    except Exception as e:
        return jsonify({"is_success": False, "error": f"Internal server error: {str(e)}"}), 500
